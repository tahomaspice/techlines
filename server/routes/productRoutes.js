...
const createNewProduct = asyncHandler(async(req, res) => {
    ...
    const newProduct = await Product.create({
      brand, name, category, stock, price, 
      image: '/images/' + image, productIsNew, description,
    });
    ...
    if(newProduct) {
        ...
        res.status(404)
        throw new Error('Product not uploaded')
    }
});
...
productRoutes.route('/:category').get(getCategory);
