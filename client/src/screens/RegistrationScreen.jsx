import { Box, Button, Container, FormControl, Heading, HStack, Stack, Text, useBreakpointValue, useColorModeValue as mode, 
    Alert, AlertTitle, AlertIcon, AlertDescription, useToast, } from '@chakra-ui/react';
...
const RegistrationScreen = () => {
  ...
  return (
    <Formik
        ...
        email: Yup.string().email('Invalid email').required('A valid email address is required'),
        password: Yup.string().min(1, 'Password is too short').required('Password is required'),
        confirmPassword: Yup.string().min(1, 'Password is too short').required('Password is required')
    ...
    onSubmit={(values) => {
        ...
        {(formik) => (
            <Container maxW='lg' py={{base: '12', md: '24' }} px={{base: '0', md: '24' }} minH='4xl'>
                <Stack spacing='8' >
                    <Stack spacing='6'>
